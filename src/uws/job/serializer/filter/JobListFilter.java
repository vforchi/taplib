...
/**
 * <p>Let filter a list of jobs according to the filter parameters given in an {@link HttpServletRequest}.</p>
 * 
 ...
 * 	If no PHASE filter is specified, a default one rejecting ARCHIVED filter is set by default.
 * 	This is specified in UWS 1.1 standard so that being backward compatible with the version 1.0 of the standard
 * 	in which no ARCHIVED phase existed.
 * </i></p>
 * 
...
public class JobListFilter {
	...
	/** List of all job filters to apply. All of these filters must match in order to keep a job. */
	 ...
	 * <i>If NULL, jobs are merely added at the end of the list: no order.</i> */
	protected Comparator<UWSJob> sortComp = null;

	/** Indicate how many of the first jobs of the list {@link #jobList} must be really retained.
	 * <i>If negative or null, all jobs are of the filtered list are returned.</i> */
	protected int topSize = -1;
	/** Indicate if the jobs of the {@link #jobList} list must be returned from the beginning to the end (<code>false</code>)
	 * or from the end to the beginning (<code>true</code>).
	 * <i>If {@link #topSize} have a positive and not null value, this attribute will then apply on the {@link #topSize} first jobs:
	 * if <code>false</code> jobs from 0 to {@link #topSize} are returned, otherwise they are returned from {@link #topSize} to 0.</i> */
	protected boolean reverseOrder = false;

	/**
	 * Empty constructor. No filter and no sorting is done here.
	 * All jobs given to the {@link #filter(Iterator)} function will then be allowed.
	 */
	protected JobListFilter(){}

	/**
	 * <p>Build a filter for a whole list of jobs.
	 * This filter will actually be composed of several Job filters,
	 * in function of the HTTP-GET parameters specified in the given HTTP request.</p>
	 * 
	 * <p>Only the following HTTP-GET parameters will generate a job filter:</p>
	 * <ul>
	 * 	<li><b>PHASE</b>: a single legal phase is expected. Only jobs having this phase will pass through this filter.<br/>
	...
	public JobListFilter(final HttpServletRequest request) throws UWSException{
				...
				for(String p : values){
					if (p != null){
						try{
							...
							// create/replace the AfterFilter if the date is more recent:
		...
		// Set the PHASES filter (if no filter is specified, a default one forbidding ARCHIVED jobs is set):
		if (phasesFilter != null)
	...
	/**
	 * <p>Add the given job in the temporary internal list of filtered jobs by preserving the specified sorting.</p>
	 * 
	 * <p>
	 * 	This function added the job at the end of the list IF no sort is required.
	 * 	Otherwise, a binary search is performed and the job is added at the right place
	 * 	in the list so that keeping the list sorted.
	 * </p>
	 * 
	 * @param job	The job to keep in the displayable job list.
	 */
	...
	public Iterator<UWSJob> filter(final Iterator<UWSJob> jobList){
	...
	/**
	 * <p>This iterator is designed to return just the N first items of the given list.</p>
	 * 
	 * <p>
	...
	protected final static class TopIterator implements Iterator<UWSJob> {
		...
		/** Indicate whether the topSize items must be read in the given order or in the reverse one. */
		...
		/**
		 * Create an iterator which will read the <code>topSize</code> first item of the given list.
		 * 
		 * @param joblist	List of jobs to return.
		 * @param topSize	Number of items to read from the beginning of the list.
		 * @param reverse	<code>true</code> if the <code>topSize</code> first items must be read in the given order,
		 *               	<code>false</code> if they must be read in the reverse order.
		 */
